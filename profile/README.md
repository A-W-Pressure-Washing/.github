<!--
A&W Pressure Washing — Organization Profile
Tip: GitHub org profiles render markdown; HTML is allowed (no scripts).
Keep images under ~2–4MB total to load fast.
-->

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" src="https://r2.fivemanage.com/NFGS8AhEJAk5KA8TseWa1/pressurewash-ezgif.com-video-to-gif-converter.gif">
    <!-- <source media="(prefers-color-scheme: dark)" srcset= "./assets/banner-dark.gif"> -->
    <img alt="A&W Pressure Washing — Middle Tennessee" src="https://r2.fivemanage.com/NFGS8AhEJAk5KA8TseWa1/pressurewash-ezgif.com-video-to-gif-converter.gif" width="100%" />
  </picture>
</p>

<h1 align="center">A&W Pressure Washing &amp; Cleaning</h1>
<p align="center">
  Middle Tennessee • Residential &amp; Commercial • Eco-Friendly • Veteran-Owned
</p>

<p align="center">
  <img alt="Veteran Owned" src="./assets/badge-veteran-owned.svg" height="28">
  <img alt="Insured" src="./assets/badge-insured.svg" height="28">
  <img alt="Tennessee Counties Served" src="./assets/badge-tn-served.svg" height="28">
</p>

<p align="center">
  <a href="https://awpressurewashing.pages.dev/estimate" target="_blank" rel="noopener noreferrer">
    <img alt="Get a Free Estimate" src="./assets/cta-estimate.svg" height="40">
  </a>
</p>

<p align="center">
  <a href="mailto:estimate.awpressurewashing@gmail.com">estimate.awpressurewashing@gmail.com</a>
  &nbsp;•&nbsp;
  <a href="tel:+1-931-259-7963">(931) 259-7963</a>
</p>


---

## 👋 Who We Are

**A&W Pressure Washing** is a Middle Tennessee, veteran-owned small business delivering **safe, professional exterior cleaning** with modern, low-pressure/soft-wash techniques. From vinyl siding and roofs to driveways, patios, gutters, and commercial storefronts, we combine **industry best practices** with **customer-first service**.

- ✅ Fully insured
- ♻️ Eco-friendly detergents & runoff awareness
- 🧰 Professional-grade equipment
- 🕒 Reliable scheduling and confirmations

> Want to collaborate on tooling (estimators, booking, or workflow automation)? We maintain public repos and welcome PRs!

---

## 🧩 What Lives in This Org

This org hosts code and assets for:

- **Website & Landing Pages** — Static site + Pages/Workers deployment
- **Booking & Scheduling** — Appointment requests, calendar integrations, reminders
- **Ops Utilities** — Quote calculators, invoice templates, SOPs, safety docs
- **Marketing** — Open Graph/SEO meta, image assets, reviews badges
- **Data & Docs** — Service area maps, price guides, checklists

> See pinned repositories for quick access.

---

## 🧼 Services

- House siding (soft wash)
- Roof cleaning (asphalt shingle/metal)
- Driveways, walkways, patios
- Decks & fences
- Gutter cleaning & brightening
- Commercial storefronts & lots

<details>
  <summary><b>Safety & Method</b></summary>

- **Soft wash** for siding/roofs to avoid damage
- Pre-rinse landscaping, use plant-safe practices
- Correct dilution, dwell, and rinsing times
- Post-treat concrete for organic staining
</details>

---

## 🗺️ Service Area
<p>
  <img alt="Service Area Map — Warren, DeKalb, Van Buren & surrounding Middle Tennessee counties" src="https://github.com/user-attachments/assets/ac0fcb88-3b2d-4886-b67d-3be401e69a49" />
</p>

- **Primary counties:** Warren, DeKalb, Van Buren  
- **Also serving:** surrounding Middle Tennessee by appointment

---

## ⭐ Before & After

<table>
  <tr>
    <td><img src="https://edwardspressurewashing.com/wp-content/uploads/2022/06/fence-restoration-in-progress-scaled.jpeg" alt="Before/After 1" width="100%"></td>
    <td><img src="https://img1.wsimg.com/isteam/ip/c968a4d6-9c01-401f-b433-e86b15d62b8b/Pressure%20Washing%20in%20Sarasota%20(1).jpg/:/cr=t:0%25,l:0%25,w:100%25,h:100%25/rs=w:1280" alt="Before/After 2" width="100%"></td>
  </tr>
</table>

> More transformations in our portfolio repos and social galleries.

---

## 💬 Reviews

> “Friendly, on time, and the house looks brand new.”  
> “Clear quote, fair price, phenomenal results.”

We showcase verified reviews on our site and social pages.

---

## 🗓️ Booking & Estimates

- **Instant Estimate:** <a href="https://awpressurewashing.pages.dev/estimate" target="_blank">Request online</a>  
- **Phone/Text:** <a href="tel:+1-931-259-7963">(931) 259-7963</a>  
- **Email:** <a href="mailto:estimate.awpressurewashing@gmail.com">estimate.awpressurewashing@gmail.com</a>

You’ll receive confirmation plus optional reminder notifications.

---

## 🧾 Licenses & Insurance

- General liability insurance: **Active**
- Local compliance: **Middle Tennessee jurisdictions**
- Environmental handling: **Best practices observed**

Documentation available upon request.

---

## 🤝 Partnerships & Collabs

We partner with realtors, property managers, HOAs, and contractors for scheduled maintenance, turn-ready cleanings, and seasonal refreshes.  
**Contact:** estimate.awpressurewashing@gmail.com

---

## 🧠 Tech Behind the Business (Open Source)

We maintain lightweight, maintainable codebases:

- Static site (Pages) with modern meta tags and local SEO
- Booking forms → email/webhook alerts
- Quote calculator templates & invoice templates (Google Sheets)
- GitHub Actions for linting/formatting and asset checks

> Interested in contributing? See CONTRIBUTING.md in each repo.

---

## 🔗 Social & Links

- Website: https://awpressurewashing.pages.dev  
- Facebook: https://facebook.com/awpressurewashing  
- Instagram: https://instagram.com/awpressurewashing  
- X (Twitter): https://x.com/awpressurewash  
- Discord: [https://discord.gg/A&WPressureWashing](https://discord.gg/zqJt8cQ3um)

---

<p align="center">
  <sub>© A&amp;W Pressure Washing &amp; Cleaning — Middle Tennessee</sub><br/>
  <sub><a href="https://awpressurewashing.pages.dev/privacy">Privacy</a> • <a href="https://awpressurewashing.pages.dev/terms">Terms</a></sub>
</p>
